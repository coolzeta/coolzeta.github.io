...
export default function Dapp1() {
  ...
  const [zetaCoinContract, setZetaCoinContract] = useState<Contract<typeof ZetaCoinABI> | null>(
    null
  );
  const [vaultContract, setVaultContract] = useState<Contract<typeof VaultABI> | null>(null);
  const [priceOracleContract, setPriceOracleContract] = useState<Contract<
    typeof PriceOracleABI
  > | null>(null);
    ...
    return (
      <Box
        sx={{
          ...
          height: '100vh',
          background:
            'radial-gradient(circle at 50% 20%, rgba(184,255,97,.12), transparent 35%), #080b09',
        }}
      >
        <Card
          sx={{
            ...
            maxWidth: 420,
            textAlign: 'center',
            border: '1px solid rgba(184,255,97,.18)',
  ...
  const transferZetaCoin = async () => {
    if (zetaCoinContract && web3 && transferAmount) {
      ...
      try {
        await zetaCoinContract.methods
          .transfer(
  ...
  const borrow = async () => {
    if (vaultContract && web3 && borrowAmount) {
      ...
      try {
        await vaultContract.methods
          .borrowZeta(web3.utils.toWei(borrowAmount, 'ether'))
          .send({ from: account });
      } catch (error) {
  ...
  const repay = async () => {
    if (vaultContract && web3 && repayAmount) {
      ...
      try {
        await vaultContract.methods
          .repayZeta(web3.utils.toWei(repayAmount, 'ether'))
          .send({ from: account });
      } catch (error) {
  ...
  return (
    <Box
      sx={{
        minHeight: '100vh',
        background:
          'radial-gradient(circle at 50% 8%, rgba(184,255,97,.10), transparent 32rem), linear-gradient(180deg, #080b09, #0b100c)',
        py: { xs: 5, md: 9 },
        px: 2,
      }}
    >
      <Box sx={{ maxWidth: 600, mx: 'auto' }}>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          <Box
            sx={{
              ...
              placeItems: 'center',
              borderRadius: '50%',
              border: '1px solid rgba(184,255,97,.25)',
              bgcolor: 'rgba(184,255,97,.05)',
              boxShadow: '0 0 55px rgba(184,255,97,.12)',
            }}
          >
          ...
          <Typography variant="body1" sx={{ color: 'rgba(238,244,232,.5)', mt: 1 }}>
            去中心化抵押借贷平台
          </Typography>
        </Box>
        ...
        <Card
          sx={{
            borderRadius: '3px 32px 3px 3px',
            border: '1px solid rgba(184,255,97,.16)',
            boxShadow: '0 28px 90px rgba(0,0,0,.35)',
            overflow: 'hidden',
            background: theme => alpha(theme.palette.background.paper, 0.82),
            ...
            {activeTab === 0 && (
              <Box sx={{ textAlign: 'center' }}>
                ...
                <Typography variant="h6" color="text.secondary" sx={{ mb: 3 }}>
                  ≈ $
                  {(vaultInfo?.collateralAmountOfVault
                    ? vaultInfo.collateralAmountOfVault * ethUsdValue
                    : 0
                  ).toFixed(2)}
                </Typography>
